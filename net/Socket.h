...
inline void Socket::setTcpNoDelay(bool on){
    int optval = on ? 1 : 0;
    ::setsockopt(
        sockfd_, IPPROTO_TCP, TCP_NODELAY, &optval,
        sizeof(optval));  // 设置socket选项，IPPROTO_TCPTCP层，TCP_NODELAY是否禁用Nagle
}
inline void Socket::setReuseAddr(bool on){//端口复用，重启服务器不出错
    int optval = on ? 1 : 0;
